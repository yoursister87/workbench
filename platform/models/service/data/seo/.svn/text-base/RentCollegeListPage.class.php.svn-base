 ...
 * @file                 $RentCollegeListPage.class.php$
 * @lastChangeBy         $wanyang$
 * @lastmodified         $2014-09-18$
 * @copyright            Copyright (c) 2014, www.ganji.com
 */
class Service_Data_Seo_RentCollegeListPage implements Service_Data_Seo_InterfacePath
{
    protected $prefix = 'RentCollege_';
    protected $Page;
    ...
    protected function generateTemplateTitle(){
        ...
        $key = '';
        if ($this->Page->getParams('college')) {

            $key = 1;
        }
        if (!$key && $this->Page->getFilterArrayWithAgentTitle()) {
            $key = 2;
        }
        if (!$key) {
            $key = 3;
